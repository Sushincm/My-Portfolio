...
export default function GameApp({ setModal, onCollectSkill, setEngineReady, restartRef }) {
  ...
  useEffect(() => {
    ...
    /* ── SHARED INPUT STATE ── */
    // Single object written by BOTH keyboard handlers AND mobile button handlers
    ...
    k.scene('game', () => {
      ...
      // Game loop — reads from shared `input`
      k.onUpdate(()=>{
        ...
        k.camPos(player.pos.x+280, H/2);
        if(player.pos.y>H+200) player.pos=k.vec2(SPAWN_X,SPAWN_Y);
      });

      // Mario-style flying coin animation
      const spawnCoinAnim = (pos) => {
        // Start from the coin's world position, convert to screen-relative
        const fly = k.add([
          k.sprite('coin'),
          k.pos(pos.x, pos.y),
          k.scale(3),
        ...
        fly.onUpdate(() => {
          ft += k.dt();
          fly.pos.y -= 140 * k.dt(); // rise upward
          fly.opacity = Math.max(0, 1 - ft * 1.4); // fade
      ...
      player.onCollide('pcoin', c=>{
        ...
        k.destroy(c);
        spawnCoinAnim(pos);
        collectSkillRef.current(); // increments coin counter
        c.action();
      });
      ...
      // Minimal HUD strip (bottom of guide area)
      k.onDraw(()=>{
    ...
    // Expose restart so the WinModal "Play Again" button can call it
    if (restartRef) {
      restartRef.current = () => {
        k.go('game');
      };
    ...
    ctrl.id = 'touch-ctrl';
    ctrl.style.cssText = 'position:fixed;bottom:20px;left:0;right:0;display:flex;justify-content:space-between;padding:0 20px;z-index:30;pointer-events:none;user-select:none;';
    ctrl.innerHTML = `
      <div style="display:flex;gap:12px">
        <button id="tl" style="pointer-events:all;width:72px;height:72px;border-radius:14px;background:rgba(0,0,0,0.6);border:3px solid rgba(255,255,255,0.65);font-size:30px;color:#fff;cursor:pointer;">◀</button>
        <button id="tr" style="pointer-events:all;width:72px;height:72px;border-radius:14px;background:rgba(0,0,0,0.6);border:3px solid rgba(255,255,255,0.65);font-size:30px;color:#fff;cursor:pointer;">▶</button>
      </div>
      <button id="tj" style="pointer-events:all;width:88px;height:72px;border-radius:14px;background:rgba(220,60,0,0.9);border:3px solid rgba(255,180,0,1);font-size:11px;color:#fff;cursor:pointer;font-family:'Press Start 2P',monospace;">JUMP</button>
    `;
    ...
    // Mobile buttons write to the SAME `input` object the game loop reads
    tl.onpointerdown = e => { e.preventDefault(); input.left  = true; };
    tl.onpointerup   = e => { e.preventDefault(); input.left  = false; };
    tl.onpointerleave= () => { input.left  = false; };
    tr.onpointerdown = e => { e.preventDefault(); input.right = true; };
    tr.onpointerup   = e => { e.preventDefault(); input.right = false; };
    tr.onpointerleave= () => { input.right = false; };
    tj.onpointerdown = e => { e.preventDefault(); input.jump  = true; };
    tj.onpointerup   = e => { e.preventDefault(); input.jump  = false; };
    ...
    return () => {
      ...
      document.getElementById('touch-ctrl')?.remove();
      k.quit(); canvas.remove(); initialized.current = false;
    };
  }, []);

  return <div ref={containerRef} style={{ position:'fixed', inset:0, zIndex:0, overflow:'hidden' }} />;
}
